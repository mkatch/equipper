...
export namespace Schema {
   ...
   *   3. A {@link UnionMember} schema for records that are members discriminated
   *     union.
   * 
  ...
  function validateInternal(schema: Schema<any>, obj: any, c: string[]): void {
    ...
    if (schema instanceof Array) {
      ...
      return obj.forEach(
        (item, i) => validateInternal(subSchema, item, [...c, `[ ${i} ]`]));
    }
    ...
    if (schema instanceof Union || schema instanceof UnionMember) {
      ...
      if (schema instanceof Union) {
        ...
        if (!member) {
          validationFailed(
            cKey,
            `expected one of \`${Object.keys(schema.members).join(' | ')}\`, ` +
            `got \`${discriminatorValue}\``
          );
        }
        ...
        if (discriminatorValue !== schema.discriminatorValue) {
          validationFailed(
            cKey,
            `expected \`${schema.discriminatorValue}\`, ` +
            `got \`${discriminatorValue}\``
          );
        }
